# This is heavily inspired by musicbrainzngs library
...
from .caa import get_image_front
...
VALID_INCLUDES = {
    ...
    "label": ["releases", "discids", "media", "aliases", "annotation"]  # Subqueries
    + RELATION_INCLUDES
    + TAG_INCLUDES
    + RATING_INCLUDES,
    ...
    + RATING_INCLUDES
    + RELATION_INCLUDES,
    "series": ["annotation", "aliases"] + RELATION_INCLUDES + TAG_INCLUDES,
    "work": ["aliases", "annotation"]
    + TAG_INCLUDES
    + RATING_INCLUDES
    + RELATION_INCLUDES,
...
VALID_BROWSE_INCLUDES = {
    ...
    "recording": ["artist-credits", "isrcs", "work-level-rels"]
    + TAG_INCLUDES
    + RATING_INCLUDES
    + RELATION_INCLUDES,
    "release": [
...
def set_useragent(app, version, contact=None):
    ...
    if contact is not None:
        _useragent = f"{app}/{version} ( {contact} )"
    else:
        _useragent = f"{app}/{version}"
...
def _mb_request(
    ...
    """Makes a request for the specified `path` (endpoint) on /ws/2."""
    ...
    if _useragent == "":
        raise UsageError(
            ...
            'set_useragent("application name", "application version",'
            ' "contact info (preferably URL or email for your application)")'
        )
    ...
    url = f"{scheme}://{hostname}/ws/2/{path}"
    ...
    elif auth_required == AUTH_IFSET and user:
        auth = HTTPDigestAuth(user, password)

    logger.debug("%s request for %s", method, url)
    logger.debug("requesting with UA %s", _useragent)
    retry_num = 0
...
def _do_mb_query(entity, id, includes=[], params={}):
    """Make a single GET call to the MusicBrainz json API. `entity` is a
...
def get_area_by_id(id, includes=[], release_status=[], release_type=[]):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params(
        "area", includes, release_status, release_type
    )
...
def get_instrument_by_id(id, includes=[], release_status=[], release_type=[]):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params(
        "instrument", includes, release_status, release_type
    )
...
def get_place_by_id(id, includes=[], release_status=[], release_type=[]):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params(
        "place", includes, release_status, release_type
    )
...
def get_event_by_id(id, includes=[], release_status=[], release_type=[]):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params(
        "event", includes, release_status, release_type
    )
...
def get_releases_by_discid(id, includes=[], toc=None, cdstubs=True, media_format=None):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params(
        "discid", includes, release_status=[], release_type=[]
    )
    if toc:
        params["toc"] = toc
    if not cdstubs:
...
def _browse_impl(
    entity, includes, limit, offset, params, release_status=[], release_type=[]
):
...
def browse_events(
    area=None, artist=None, place=None, includes=[], limit=None, offset=None
):
    """Get all events linked to a area, a artist or a place.
    You need to give one MusicBrainz ID.

    *Available includes*: {includes}"""
    params = {"area": area, "artist": artist, "place": place}
...
def browse_releases(
    ...
    *Available includes*: {includes}"""
    ...
    return _browse_impl(
        "release", includes, limit, offset, params, release_status, release_type
    )
...
def browse_release_groups(
    ...
    *Available includes*: {includes}"""
    params = {"artist": artist, "release": release}
    return _browse_impl(
        "release-group", includes, limit, offset, params, [], release_type
    )
...
def _do_collection_query(collection, collection_type, limit, offset):
    ...
    return _do_mb_query("collection", f"{collection}/{collection_type}", [], params)
